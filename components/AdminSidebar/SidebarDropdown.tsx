        ...
        {item.map((dropdownItem: any, index: number) => (
          <li key={index}>
            <Link
              ...
              className={`relative flex items-center rounded-[7px] px-3.5 py-2 font-medium duration-300 ease-in-out ${
            ...
            >
              {dropdownItem.icon && <span className="flex-shrink-0 mr-2">{dropdownItem.icon}</span>}
              {dropdownItem.label}
              {dropdownItem.pro && (
